...
function increaseCount(current, next, index){
        ...
        setTimeout(function() {
            console.log("before", next[index], current[index]);
            current[index].innerText = next[index].innerText;
            next[index].classList.remove("animate");
           
            parseInt(next[index].innerText ++) ;
            console.log("after", next[index], current[index]);
          
            if(next[index].innerText > 9) { 
                next[index].innerText = 0;
            }
        }, 500);
